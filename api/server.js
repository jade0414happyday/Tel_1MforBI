...
const VERSION = "tel-api-profile-0.5.1";
...
function normBankCode(s) {
  // 對齊 v_fact_txn_pretty 的 bank_code：UPPER(REPLACE(REPLACE(bank_name,' ',''),'-',''))
  return String(s ?? "")
    .trim()
    .toUpperCase()
    .replace(/\s+/g, "")
...
async function profileTxns(q) {
  const bookRaw = q.book;
  const bankRaw = q.bank_code;
  const from = q.from;
  const to = q.to;
  ...
  if (!book || !bank_code || !isYmd(from) || !isYmd(to)) {
    return {
      ...
      body: {
        ...
        version: VERSION,
        query: { book: bookRaw, bank_code: bankRaw, from, to },
      },
    };
  }
  ...
  try {
    ...
    // FIX#2: 這三個 const 只能宣告一次（你之前重複貼了）
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    return sendJson(res, 404, { error: "not_found", version: VERSION });
  } catch (e) {
